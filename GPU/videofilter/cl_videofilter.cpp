...
int main(int, char**)
{
    ...
    while (true) {
      ...
      temp1_buf = clCreateBuffer(context, heckError(status, "Failed to create buffer for output"), CL_MEM_READ_WRITE, ROWS*COLS*sizeof(float), NULL, &status);
      checkError(status, "Failed to create buffer for temp1");

      temp2_buf = clCreateBuffer(context, heckError(status, "Failed to create buffer for output"), CL_MEM_READ_WRITE, ROWS*COLS*sizeof(float), NULL, &status);
      checkError(status, "Failed to create buffer for temp1");

      // Map input array
      float* input = (float *)clEnqueueMapBuffer(queue, input_buf, CL_TRUE,
          CL_MAP_WRITE,0, ROWS*COLS*sizeof(float), 0, NULL, write_event, &status);
      checkError(status, "Failed to map input");

      // Map output array
      float* output = (float *)clEnqueueMapBuffer(queue, output_buf, CL_TRUE,
          CL_MAP_READ, 0, ROWS*COLS*sizeof(float),  0, NULL, NULL, &status);
...
float * applyConvolution(float* input, float* convolution) {
  ...
  status = clEnqueueReadBuffer(queue, output_buf, CL_TRUE, 0, N*N* sizeof(float), output, 1, &kernel_event, &finish_event);
}
